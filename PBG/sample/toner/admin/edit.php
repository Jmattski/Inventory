...
<html>
	...
	<body>
			...
			<nav class="w3-sidebar w3-bar-block w3-khaki w3-animate-left w3-text-black w3-collapse w3-top w3-center" style="z-index:3;width:300px;font-weight:bold" id="mySidebar">
					...
					<a class="w3-bar-item w3-button" href="toner.php" onclick="w3_close()">NEW ITEM</a>	
			</nav>
		...
		<div class="w3-overlay w3-hide-large w3-animate-opacity" onclick="w3_close()" style="cursor:pointer" title="close side menu" id="myOverlay"></div>
			...
			<form action ="connection.php" method ="post">
				<?php $results = mysqli_query($db, "SELECT * from toner WHERE id=$id") ?>
					<?php while ($row = mysqli_fetch_array($results)) {?>
						<table class="table table-hover table-resposive table-bordered">
							<tr>
								<td>ID</td>
								<td><input type="text" name="id" class="form-control" value="<?php echo $id; ?>" readonly></td>
							</tr>
						
							<tr>
								<td>TONER NAME</td>
								<td><input type="text"	name="tname" class="form-control" value="<?php echo $row['t_name']; ?>" required></td>
							</tr>
							<tr>
								<td>DESCRIPTION</td>
							...
							<tr>
								<td>PRINTER NAME</td>
								<td><input type="text" name="pname" class="form-control" value="<?php echo $row['p_name']; ?>" required></td>
							</tr>
							<tr>
								<td>LOCATION</td>
							...
							<tr>
								<td>PURCHASE DATE</td>
								<td><input type="date" name="purchase" class="form-control" value="<?php echo $row['p_date']; ?>"></td>
							</tr>
							<tr>
								<td>RELEASE DATE</td>
								<td><input type="date" name="release" class="form-control" value="<?php echo $row['r_date']; ?>"></td>
							</tr>
							<tr>
								<td>QUANTITY</td>
					...
					<a href="ink.php" class="btn btn-primary">NO</a>
			</form>
